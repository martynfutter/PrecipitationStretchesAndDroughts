...
calculate_stretched_precip <- function(p_offset, z, threshold, stretch_factor, a, b, c, d) {
  if (z >= threshold) {
    ...
    # Formula (v): For z < t
    x <- z / threshold
    p_prime <- p_offset * (1 - (c * (x^a * (1 - x)^b)))
  }
...
stretch_precipitation <- function(daily_weather_file = DAILY_WEATHER_FILE,
  ...
  cat(paste("  For z < ", threshold, " (formula v):\n", sep = ""))
  cat(paste("    x = z / ", threshold, "\n", sep = ""))
  cat(paste("    p' = p_offset × (1 - (", round(c, 4), " × (x^", round(a, 4), " × (1-x)^", round(b, 4), ")))\n\n", sep = ""))
...
#   (v)   For z < t: x=z/t, p'=p_offset*(1-(c*(x^a*(1-x)^b)))
